...
export function OnboardingTour({ steps, currentStep, isActive, onNext, onBack, onSkip, onComplete }: Props) {
  ...
  const getTooltipStyle = (): React.CSSProperties => {
    ...
    const bottomOffset = isMobile ? 72 : 16; // Extra space for BottomNav
    const tooltipH = 180;
    // Tall element (e.g. sidebar): position to the right
    if (pos.height > vh * 0.5 && !isMobile) {
      return { position: "fixed", top: Math.min(pos.top + 60, vh - tooltipH - bottomOffset), left: pos.left + pos.width + 16, zIndex: 10001 };
    }
    // Normal element: position below, clamped to viewport
    const left = isMobile ? 16 : Math.max(16, Math.min(pos.left, window.innerWidth - 340));
    const top = Math.min(pos.top + pos.height + 12, vh - (isMobile ? 240 : tooltipH) - bottomOffset);
    return { position: "fixed", top, left, zIndex: 10001 };
  };
  ...
  return (
    <>
      ...
      {hasTarget && pos && (
        <div
          className="fixed z-[10000] rounded-lg ring-2 ring-primary/80 ring-offset-1 pointer-events-none transition-all duration-300"
          style={{ top: pos.top - 2, left: pos.left - 2, width: pos.width + 4, height: pos.height + 4 }}
        />
      )}
      ...
      <div
        ref={tooltipRef}
        className="w-[calc(100vw-32px)] sm:w-[320px] rounded-lg border border-border bg-card p-4 shadow-xl"
        style={tooltipStyle}
